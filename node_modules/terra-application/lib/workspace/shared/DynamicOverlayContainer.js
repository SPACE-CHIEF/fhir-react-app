...
function setLayerInert(inertElement, focusTriggers, focusTriggerKey, focusAnchorElement) {
  ...
  var _document = document,
      activeElement = _document.activeElement; // If the element to be inerted contains or is the current active element,
...
function removeLayerInert(inertElement, focusTriggers, focusTriggerKey, focusAnchorElement) {
  ...
  inertElement.inert = false; // If a focus trigger has been cached for this element, and if the element still contains or
...
var DynamicOverlayContainer = function DynamicOverlayContainer(_ref) {
  ...
  var focusTriggersRef = _react.default.useRef({}); // The refs for each overlay are kept in a local variable to ensure
  // it expires after each render to ensure that removed overlays do not
  // persist in memory.


  var overlayRefs = []; // This effect ensures that the proper layers are rendered
