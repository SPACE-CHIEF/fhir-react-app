...
exports.breakpointFilter = exports.default = void 0;
...
var useElementSize = function useElementSize(elementRef, updateFilter) {
  ...
  var cachedUpdateFilterRef = _react.default.useRef(updateFilter); // An active animation frame id is stored in this ref so it can be cancelled, if necessary.


  var animationFrameRef = _react.default.useRef(); // The last-returned rect value is stored in this ref so that comparisons can be made against
  // it without reading the state value in the layout effect and re-binding the resize observer.


  var reportedRectRef = _react.default.useRef(); // The rect stored in this state is returned to the consuming component.
  ...
  _react.default.useLayoutEffect(function () {
    ...
    var resizeObserver = new _resizeObserverPolyfill.default(function (entries) {
      animationFrameRef.current = window.requestAnimationFrame(function () {
        animationFrameRef.current = null; // We do not report size changes for elements that have been removed from the DOM.
        ...
        var newRect = processRect(entries[0].target.getBoundingClientRect()); // If the consuming component provides a filter function, we execute it with the current and
    ...
    resizeObserver.observe(elementRef.current); // The initial size is kicked off prior to the ResizeObserver initialization.
