...
var useNotificationBanners = function useNotificationBanners() {
  ...
  var registeredBanners = _react.default.useRef({}); // The updateBannerState ref stores the update state function used to manage the banners rendered in the NotificationBanners component.
  ...
  var useNotificationBannerExports = _react.default.useMemo(function () {
    ...
    var NotificationBanners = function NotificationBanners(_ref2) {
      ...
      var _React$useState = _react.default.useState({}),
          ...
          setBannerState = _React$useState2[1]; // The container reference points to the notification region so we
      // can move focus there when necessary.


      var containerRef = _react.default.useRef(); // We track the last rendered set of banners so that we can determine
      // when banners are added/removed at render time.


      var lastRenderedBannersRef = _react.default.useRef([]); // We track the last read removal message to ensure that duplicates are
      // known and updated to work around some VoiceOver issues.


      var lastReadAddedBanner = _react.default.useRef(); // We track the last read removal message to ensure that duplicates are
      // known and updated to work around some VoiceOver issues.


      var lastReadRemovedBanner = _react.default.useRef(); // We track identifiers for banners explicitly dismissed by the user
      // so that we can appropriately read the result.


      var dismissalRefs = _react.default.useRef({});

      var forceUpdate = (0, _useForceUpdate.default)(); // Set the updateBannerState ref to the update state function. This ties the state updates to the `useNotificationBanners` hook,
